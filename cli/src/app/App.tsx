...
export function App() {
  ...
  useKeyboard((key) => {
    ...
    if (screen === "home") {
      if (codeInput !== null) {
        if (key.name === "escape") setCodeInput(null)
        else if (key.name === "backspace" || key.name === "delete") setCodeInput((current) => current?.slice(0, -1) ?? "")
        else if (key.name === "return" || key.name === "enter") {
          const code = codeInput
          setHomeNotice(`Loading ${code.toUpperCase()}…`)
          void fetchPuzzleByCode(code).then((puzzle) => {
            setPuzzles((current) => current.some(({ id }) => id === puzzle.id) ? current : [...current, puzzle])
            setHomeSelection((current) => current)
            setCodeInput(null)
            setHomeNotice(`Loaded ${puzzle.name} · ${code.toUpperCase()}`)
          }).catch((error) => setHomeNotice(error instanceof Error ? error.message : "Could not load puzzle"))
        } else if (key.sequence && /^[a-z0-9]$/i.test(key.sequence) && codeInput.length < 8) setCodeInput(codeInput + key.sequence.toUpperCase())
        return
      }
      if (key.name === "t") {
        openTutorial()
  ...
  if (screen === "creator") {
    const creatorTooSmall = dimensions.width < 70 || dimensions.height < 23
    return (
      <box width="100%" height="100%" position="relative" alignItems="center" justifyContent="center" backgroundColor={mono.background}>
        <CreatorScreen
          disabled={creatorTooSmall}
          theme={mono}
        ...
        />
        {creatorTooSmall && (
          <box position="absolute" zIndex={20} width="100%" height="100%">
            {expandWindow}
          </box>
        )}
      </box>
